...
class MedicationOrder extends Model
{
    ...
    public function generateNextDose(): ?MedicationAdministration
    {
        if ($this->administrations()->where('status', 'pending')->exists()) {
            return null;
        }

        $duration = $this->duration ?? $this->dailyDoseCount();

        $actionedCount = $this->administrations()
