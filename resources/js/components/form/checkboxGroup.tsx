import { Box, Checkbox, FormControl, FormControlLabel, FormGroup, FormHelperText, FormLabel, Grid, type CheckboxProps, type FormGroupProps } from '@mui/material'
...
type Props<T extends FieldValues = FieldValues> = {
   ...
   checkboxProps?: Omit<CheckboxProps, 'checked' | 'defaultChecked' | 'disabled' | 'name' | 'onChange' | 'required' | 'slotProps' | 'value'>
...
const CheckboxGroup = <T extends FieldValues = FieldValues>({
  ...
  const hasExclusiveValue = exclusiveValue !== undefined && selectedValues.includes(exclusiveValue)

  const handleChange = (optionValue: string | number, checked: boolean) => {
    const nextValues = checked
      ? optionValue === exclusiveValue
        ? [optionValue]
        : [...selectedValues.filter((value) => value !== exclusiveValue && value !== optionValue), optionValue]
  ...
  const renderedOptions = options.map((option, index) => {
    if (option.text || option.value === undefined) {
      return (
        <Grid key={`heading-${index}`} size={12}>
          <Box sx={{ fontSize: '1rem', fontWeight: 600, color: 'text.primary' }}>
    ...
    const optionDisabled = disabled || (hasExclusiveValue && optionValue !== exclusiveValue)
        ...
        <FormControlLabel
          control={
            <Checkbox
              ...
              name={field.name}
              value={optionValue}
              onChange={(event) => handleChange(optionValue, event.target.checked)}
              onBlur={field.onBlur}
  ...
  return (
    <FormControl component="fieldset" fullWidth error={!!error} required={!!rules?.required} disabled={disabled}>
      ...
      {grid ? (
        <Grid container spacing={0.5} aria-labelledby={label ? labelId : undefined} aria-describedby={error ? helperTextId : undefined}>
          {renderedOptions}
        </Grid>
      ) : (
        <FormGroup
