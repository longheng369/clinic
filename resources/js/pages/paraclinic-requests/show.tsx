        ...
        <Section title="General Information">
          ...
          >
            {[
              ...
              ['External Facility', request.external_facility_name],
          ...
          {request.notes && <InfoItem label="Notes" value={request.notes} />}
        </Section>
          ...
          {request.results.map((r) => (
            <Alert key={r.id} severity="info" sx={{ mb: 1 }}>
              <Typography variant="caption">
                <Calendar size={12} /> {r.result_date} · Reviewed by{' '}
                {r.reviewed_by ?? 'N/A'}
                {r.reviewed_at &&
                  ` at ${new Date(r.reviewed_at).toLocaleString()}`}
              </Typography>
              {r.result_summary && (
                <Typography>
                  <strong>Summary:</strong> {r.result_summary}
                </Typography>
              )}
              {r.doctor_interpretation && (
                <Typography>
                  <strong>Interpretation:</strong> {r.doctor_interpretation}
                </Typography>
              )}
            </Alert>
          ))}
          {(request.status === 'Waiting Result' ||
            request.status === 'Result Received') && (
