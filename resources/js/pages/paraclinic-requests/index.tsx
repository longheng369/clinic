...
const Index = () => {
  ...
  const columns: Column<IParaclinicRequest>[] = [
    { header: 'លេខស្នើសុំ', cell: (r) => r.request_number },
    {
      header: 'អ្នកជំងឺ',
      cell: (r) =>
        r.patient ? (
          <Typography component="span">
            {r.patient.khmer_last_name} {r.patient.khmer_first_name}
          </Typography>
        ) : (
    ...
    { header: 'កាលបរិច្ឆេទ', cell: (r) => r.request_date },
    {
      header: 'ស្ថានភាព',
      cell: (r) => (
        <Chip
    ...
    },
    {
      ...
      cell: (r) => (
        <Chip
          ...
          color={PAYMENT_COLORS[r.payment_status] ?? 'default'}
        />
      ),
    },
    {
    ...
    },
    {
      ...
      cell: (r) => (
        <Stack direction="row" sx={{ justifyContent: 'flex-end' }}>
          <IconButton
            onClick={() => router.visit(`/paraclinic-requests/${r.id}`)}
  ...
  const filter = (value: string, key: string, setter: (v: string) => void) => {
    setter(value);
    router.get(
      '/paraclinic-requests',
      { [key]: value || null },
      ...
      <Box sx={{ p: 4 }}>
        ...
        >
          <Box>
            ...
            <Typography color="text.secondary">
              Manage diagnostic test requests for patients
            </Typography>
          </Box>
          <Button
        ...
        >
          <TextField
            ...
            slotProps={{ input: { 'aria-label': 'Search requests' } }}
          />
          <Select
            size="small"
            value={filterStatus}
            onChange={(e) => filter(e.target.value, 'status', setFilterStatus)}
            displayEmpty
          ...
          <Select
            ...
            value={filterPayment}
            onChange={(e) =>
              filter(e.target.value, 'payment_status', setFilterPayment)
            }
            displayEmpty
