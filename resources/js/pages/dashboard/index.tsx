    ...
    <>
      ...
      <Box sx={{ p: 4 }}>
        ...
        <Paper variant="outlined" sx={{ overflow: 'hidden', borderRadius: 2 }}>
          ...
          >
            <Syringe size={20} color="var(--mui-palette-primary-main)" />
            ...
            {vaccinationDueAlerts.length > 0 && (
              <Chip
                ...
                size="small"
                color="primary"
                label={`${vaccinationDueAlerts.length} alert${vaccinationDueAlerts.length !== 1 ? 's' : ''}`}
              />
            )}
          </Stack>
          {vaccinationDueAlerts.length === 0 ? (
            ...
            >
              {overdue.length > 0 && (
                <AlertGroup
                  title={`Overdue — ${overdue.length} patient${overdue.length !== 1 ? 's' : ''}`}
              ...
              {upcoming.length > 0 && (
                <AlertGroup
                  title={`Due Within 7 Days — ${upcoming.length} patient${upcoming.length !== 1 ? 's' : ''}`}
                  icon={<Clock size={14} />}
                  alerts={upcoming}
                />
              )}
...
const AlertGroup = ({
  ...
  alerts: IDashboardVaccinationAlert[];
}) => (
  <Box sx={{ p: 2 }}>
    <Stack direction="row" spacing={1} sx={{ mb: 1, alignItems: 'center' }}>
      <Box color="error.main">{icon}</Box>
...
const AlertItem = ({ alert }: { alert: IDashboardVaccinationAlert }) => {
  ...
  return (
    ...
    >
      ...
      >
        <Box color={alert.is_overdue ? 'error.main' : 'warning.main'}>
          {alert.is_overdue ? <AlertTriangle size={16} /> : <Clock size={16} />}
        </Box>
        <Box sx={{ minWidth: 0 }}>
          ...
          <Typography variant="caption" color="text.secondary">
            {alert.vaccine_name} — Dose {alert.dose_number} (
            {alert.doses_completed}/{alert.total_doses} completed)
          </Typography>
        </Box>
      </Stack>
      <Typography
        variant="caption"
        color={alert.is_overdue ? 'error' : 'warning.main'}
