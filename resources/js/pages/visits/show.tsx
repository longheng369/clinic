...
const VisitShow = ({
  ...
  return (
    <>
      <Head
        title={`Visit - ${patient.khmer_last_name} ${patient.khmer_first_name}`}
      />
      ...
      <Box>
              ...
              return (
                <Row key={m.id}>
                  <Box>
                    ...
                    <Box>{m.route}</Box>
                    <Box>
                      {m.dosage} {m.unit} &middot; {m.interval}
                    </Box>
                  </Box>
                  <Box>
                    <Box
              ...
              <Box key={p.id}>
                ...
                {p.items.map((item) => (
                  <Row key={item.id}>
                    ...
                    <Box>{item.notes && <Box>{item.notes}</Box>}</Box>
                  </Row>
                ))}
                {p.notes && <Box>Note: {p.notes}</Box>}
              </Box>
            ))}
          </Section>
        )}
        ...
        {paraclinicRequests.length > 0 && (
          <Section
            ...
            count={paraclinicRequests.length}
          >
